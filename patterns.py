...
def pre_detection_pattern_for_cnic(camera_id: int, cam_url:str, crop: str, cam_type: str, frame_queue: mp.Queue, previously_saved_cnic: mp.Manager, card_already_in_holder: mp.Manager):
    ...
    while True:
        ...
        if not get_mp_list_object_from_cam_id(camera_id, card_already_in_holder)["status"]:
...
def post_detection_pattern_for_cnic(result: dict, previously_saved_cnic: mp.Manager, card_already_in_holder: mp.Manager):
    ...
    if len(texts) == 0:
        update_mp_list_object_from_cam_id(cam_id, card_already_in_holder, "status", None)
    ...
    previously_saved_cnic_current_camera = get_mp_list_object_from_cam_id(cam_id, previously_saved_cnic)["cnic"]

    if previously_saved_cnic_current_camera is None and cnic is not None:
        print("New card detected, previoiusly saved cnic is None and now setting it to: ", cnic)
        asyncio.run(add_data_to_database(name, n_confidence, cnic, c_confidence, all_info, timestamp, cam_id))
        update_mp_list_object_from_cam_id(cam_id, previously_saved_cnic, "cnic", cnic)
        update_mp_list_object_from_cam_id(cam_id, card_already_in_holder, "status", True)
        if cnic is not None and save_image is not None:
            save_cnic_image(save_image, cnic + ".jpg")
    elif previously_saved_cnic_current_camera is not None and cnic is not None and cnic != previously_saved_cnic_current_camera:
        print("New card detected, previoiusly saved cnic is not None and now setting it to: ", cnic)
        asyncio.run(add_data_to_database(name, n_confidence, cnic, c_confidence, all_info, timestamp, cam_id))
        update_mp_list_object_from_cam_id(cam_id, previously_saved_cnic, "cnic", cnic)
        update_mp_list_object_from_cam_id(cam_id, card_already_in_holder, "status", True)
        if cnic is not None and save_image is not None:
...
def post_detection_loop(ocr_results_queue: mp.Queue, previously_saved_cnic: mp.Manager, card_already_in_holder: mp.Manager):
...
def run_system(cams: list, frame_queue: mp.Queue, ocr_results_queue:mp.Queue, args, previously_saved_cnic: mp.Manager, card_already_in_holder: mp.Manager):
    ...
    for cam in cams:
        if cam["type"] == "cnic":
            p = mp.Process(target=pre_detection_pattern_for_cnic, 
                            args=(
                                cam["id"], 
                                cam["cam_url"], 
                                cam["crop"], 
    ...
    pocr_post = mp.Process(target=post_detection_loop, args=(
        ocr_results_queue,
        previously_saved_cnic,
        card_already_in_holder
        )
